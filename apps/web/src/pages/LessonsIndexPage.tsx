...
export function LessonsIndexPage() {
  ...
  return (
    <main className="mx-auto flex min-h-dvh max-w-3xl flex-col gap-6 px-4 py-8 md:py-12">
      ...
      {continueLesson && <ContinueLearning continueLesson={continueLesson} variant="banner" />}
      ...
      {lessons.length === 0 ? (
        ...
        <ul className="grid gap-3 md:grid-cols-2">
          {lessons.map((lesson) => {
            ...
            const isContinueTarget = continueLesson?.target.lessonId === lesson.id;
            return (
              <li key={lesson.id}>
                <Link
                  to={isContinueTarget ? continueLesson.href : `/lessons/${lesson.id}`}
                  className="block focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--cc-focus-ring)] rounded-2xl"
                >
                  <Card
                    className={`h-full transition-shadow hover:shadow-md ${isContinueTarget ? 'ring-2 ring-[var(--cc-primary)]' : ''}`}
                  >
                    ...
                    <div className="mt-2 flex flex-wrap items-center gap-2">
                      ...
                      {isContinueTarget && (
                        <span className="rounded-full bg-[var(--cc-primary)] px-2 py-0.5 text-xs font-semibold text-[var(--cc-primary-fg)]">
                          Continue
                        </span>
                      )}
                    </div>
